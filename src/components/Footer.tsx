...
export default function Footer({ content }: FooterProps) {
  const c = {
    ...
    copyrightText: content?.copyrightText || '© 2024 XVerse Solutions. All rights reserved.',
    ...
    <footer className="wa-footer">
      <div className="wa-container">
        <div className="wa-footer-grid">
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
              <div style={{ width: 36, height: 36, background: 'linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div className="wa-footer-bottom">
          ...
          <div className="wa-footer-social">
            {c.socialLinkedin && (
              <a href={c.socialLinkedin} aria-label="LinkedIn" target="_blank" rel="noopener noreferrer">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
              </a>
            )}
            {c.socialGithub && (
              <a href={c.socialGithub} aria-label="GitHub" target="_blank" rel="noopener noreferrer">
            ...
            {c.socialTwitter && (
              <a href={c.socialTwitter} aria-label="Twitter" target="_blank" rel="noopener noreferrer">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              </a>
            )}
            {c.socialDribbble && (
              <a href={c.socialDribbble} aria-label="Dribbble" target="_blank" rel="noopener noreferrer">
